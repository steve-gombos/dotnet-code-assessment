...
namespace Template.Api.IntegrationTests.Orderers
{
    public class PriorityOrderer : ITestCaseOrderer
    {
            ...
            IEnumerable<TTestCase> testCases) where TTestCase : ITestCase
        {
            ...
            {
                var priority = testCase.TestMethod.Method.GetCustomAttributes(assemblyName).FirstOrDefault()
        ...
        private static TValue GetOrCreate<TKey, TValue>(IDictionary<TKey, TValue> dictionary, TKey key)
            where TKey : struct where TValue : new()
        {
            return dictionary.TryGetValue(key, out var result) ? result : dictionary[key] = new TValue();
        }
    }
}
