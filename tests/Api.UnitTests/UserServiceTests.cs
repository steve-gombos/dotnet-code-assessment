...
namespace Template.Api.UnitTests
{
    public class UserServiceTests
    {
        ...
        public async Task RefreshUserById_Should_Return_Expected(int userId, bool expectedIsComplete, bool expectedIsActive)
        {
            // Arrange
            var mockUsers = GetMockUsers().CreateMockDbSet();
            _context.Users.Returns(mockUsers);
        ...
        private static IList<User> GetMockUsers()
        {
            ...
            {
                new User { Id = 1, IsComplete = false, IsActive = true },
                new User { Id = 2, IsComplete = false, IsActive = true },
                new User { Id = 3, IsComplete = false, IsActive = true }
            };
        }
